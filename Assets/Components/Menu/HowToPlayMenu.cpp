...
void RType::HowToPlayMenu::goToMenu(const std::string &menuName) {
    auto objs = scene.getGameObjects("Canvas" + menuName);
    auto objCurr = scene.getGameObjects("CanvasHowToPlayMenu");
    ...
    if (_found.use_count() == 0) {
        KAP_DEBUG_ERROR("Canvas called " + menuName + " not found to display it");
        return;
    }
